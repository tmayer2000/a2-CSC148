...
class PaperTree(TMTree):
    ...
    _authors:
        the authors of the given paper or an empty string if self is an internal
        node
    _doi:
        the url of the given paper or an empty string if self is an internal
        node
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        """
        if all_papers:
            temp = _load_papers_to_dict(by_year)
            TMTree.__init__(self, name, _build_tree_from_dict(temp))
        else:
...
def _dict_maker(categories: List, title: str, auth: str, cit: int, doi: str,
                parent: str) -> Dict:
    """Returns a nested dictionary representing a single paper's tree
    """
    old = None
    for cont in categories[::-1]:
        if old is None:
            old = {cont: {(title, auth, cit, doi, parent): None}}
        else:
            old = {cont: old}
    return old
...
def _merge(a: Dict, b: Dict) -> None:
    """merges two nested dictionaries of varying depth
    """
    for item in b:
        if item in a and isinstance(list(b[item].keys())[0], tuple):
            a[item][list(b[item].keys())[0]] = None
        elif item in a and isinstance(b[item], dict) and isinstance(a[item],
                                                                    dict):
            _merge(a[item], b[item])
        elif item not in a and isinstance(item, str):
            a[item] = b[item]
        else:
            a[item][b[item]] = None
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    """
    with open(DATA_FILE, 'r') as papers:
        ...
        for lines in reader:
            if lines[0] == 'Author':
                ...
                categories = lines[3].split(':')
                if by_year:
                    updated = [lines[2]]
                else:
                    updated = []
                for items in categories:
                    updated.append(items.lstrip())
                name = lines[1]
                author = lines[0]
                citation = int(lines[5])
                parent = updated[-1]
                url = lines[4]
                cur.append(_dict_maker(updated, name, author, citation, url,
                                       parent))
    for dicts in cur[1:]:
        _merge(cur[0], dicts)
    return cur[0]
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    for item in nested_dict:
        if isinstance(item, str):
            ans.append(PaperTree(item,
                                 _build_tree_from_dict(nested_dict[item])))
        elif isinstance(item, tuple):
            ans.append(PaperTree(item[0], [], item[1], item[3], item[2]))
    return ans
